//
//  GOForgotPasswordViewController.m
//  GoOut
//
//  Created by Liang GUO on 7/28/14.
...
@interface GOForgotPasswordViewController ()
...
@implementation GOForgotPasswordViewController
...
- (void)initLayout{
    ...
    //_sendCode.backgroundColor = [UIColor yellowColor];
    ...
    [_registerButton setTitle:@"登陆" forState:UIControlStateNormal];
    _registerButton.backgroundColor = BUTTON_COLOR;
    [_registerButton.layer setCornerRadius:10.0];
    [_registerButton addTarget:self action:@selector(forgotButtonEvent:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:_registerButton];
}
#pragma mark -- Event Handling
- (void)forgotButtonEvent:(UIButton*)sender
{
    if ([GOCommon isValidPhoneNumber:_phoneNumText.text]) {

        if (_passwordText.text.length >0) {
            [[GONetworkController sharedController]doforgotPWD:_phoneNumText.text authenticateCode:_passwordText.text];
//            //valid number
//            [SMS_SDK commitVerifyCode:_passwordText.text result:^(enum SMS_ResponseState state) {
//                switch (state) {
...
//                        //验证码错误请重新获取
//                        NSString* str=[NSString stringWithFormat:@"验证码错误请重新获取"];
//                        UIAlertView* alert=[[UIAlertView alloc] initWithTitle:@"登陆失败" message:str delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
//                        [alert show];
//                    }
//                        break;
//                    default:
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    switch (resultIdentifier) {
        ...
        case FORGOT_FAILURE:
        {
            [GOCommon showHUDWithText:@"无此用户"];
        }
            break;
        case FORGOT_TIME_OUT:
        {
